...
state = {
  ...
  updateCryptosList(model) {
    ...
    if (model.config.debug)
    console.log('state.samUpdate - cryptos.list :', this.data.cryptos.listNum, clist);
  },
  ...
  updateCryptosFiltered(model) {
    ...
    const cFiltered = Object.values(this.data.cryptos.list).filter( (v,i) => {
      return (v.code.toLowerCase().includes(filters.text.toLowerCase())
           || v.name.toLowerCase().includes(filters.text.toLowerCase()))
  ...
  updatePagination(model, currency) {
    ...
    const nbPages = Math.ceil(numRows / numRowsPerPage);// nombre total sur element par page
